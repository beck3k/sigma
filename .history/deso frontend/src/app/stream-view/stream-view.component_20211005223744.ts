 ...
 orderby: string;
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      this.publicKeyOfStreamer = params.get("publicKey")
      console.log(this.publicKeyOfStreamer)
      this.getStreamer();
    })
  ...
  getStreamer() {
    this.http.get(`http://149.159.16.161:3123/stream/${this.publicKeyOfStreamer}`).subscribe((data)=>{
    this.streamer = data
    // get creators - creator coin value and username
    this.backendApi.GetSingleProfile(this.globalVars.localNode, "", this.publicKeyOfStreamer).subscribe(
      (res) => {
