...
export class StreamViewComponent implements OnInit, OnDestroy {
  ...
  usernameColor(username) {
    ...
    for (var i = 0; i < username.length; i++) {
      this.hash = username.charCodeAt(i) + ((this.hash << 5) - this.hash);
      this.hash = this.hash & this.hash;
    }
    this.hash = ((this.hash % this.colors.length) + this.colors.length) % this.colors.length
    console.log(this.hash)
    return this.colors[this.hash];
  }
