...
export class StreamViewComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.destroy()
    this.data = await this.route.paramMap.toPromise()
    this.streamerUsername = this.data.get("username")
    console.log(this.streamerUsername)
    this.data = await this.backendApi.GetSingleProfile(this.globalVars.localNode, "", this.streamerUsername).toPromise()
    this.streamerProfile = this.data.Profile
