...
export class ChatComponent implements OnInit, OnChanges, DoCheck {
  ...
  constructor(private globalVars: GlobalVarsService, private webSocketService: WebSocketService,private cd: ChangeDetectorRef) { }
  prevMessages
  @Input() chatMessages = []

  mgDoChanges() {
    this.prevMessages = this.chatMessages
  }
  ngDoCheck() {
    if (this.prevMessages != this.chatMessages) {
