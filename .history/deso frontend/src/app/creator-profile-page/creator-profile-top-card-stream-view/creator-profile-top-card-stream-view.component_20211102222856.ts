...
export class CreatorProfileTopCardStreamViewComponent implements OnInit, OnDestroy {
  ...
  goToCategory() {
    this.router.navigate(["category", this.streamCategoryName], { state: { categoryId: `${this.streamCategoryId}` } })
  }
  constructor(private _globalVars: GlobalVarsService, private backendApi: BackendApiService, private router: Router, private http: HttpClient) {
