...
@Component({
  selector: 'app-manage-logged-in-users-follows-page',
  templateUrl: './manage-logged-in-users-follows-page.component.html',
  styleUrls: ['./manage-logged-in-users-follows-page.component.scss']
})
export class ManageLoggedInUsersFollowsPageComponent implements OnDestroy {
  ...
  ManageLoggedInUsersFollowsPageComponent = ManageLoggedInUsersFollowsPageComponent;
  ...
  getPage(page: number) {
    ...
    return this.backendApi
      .GetFollows(
        ...
        "",
        this.targetPublicKey /* PublicKeyBase58Check */,
        this.getEntriesFollowingPublicKey,
        fetchPubKey,
        ManageLoggedInUsersFollowsPageComponent.PAGE_SIZE
      )
      .toPromise()
      .then((res) => {
        ...
        if (sortedProfileEntries.length < ManageLoggedInUsersFollowsPageComponent.PAGE_SIZE || this.pagedKeys[page + 1] === "") {
          this.lastPage = page;
  ...
  _handleTabClick(tabName) {
    let routeSuffix;
    if (tabName === ManageLoggedInUsersFollowsPageComponent.FOLLOWING) {
      routeSuffix = RouteNames.FOLLOWING;
    } else if (tabName === ManageLoggedInUsersFollowsPageComponent.FOLLOWERS) {
    ...
    this.router.navigate(["user", routeSuffix], { queryParamsHandling: "merge" });
  }
  ...
  _setStateFromActivatedRoute(route) {
    ...
    this.targetUsername= this.globalVars.loggedInUser.ProfileEntryResponse.Username && this.globalVars.loggedInUser.PublicKeyBase58Check
    let followingParamIndex = 1;
    ...
    switch (path) {
      case this.appData.RouteNames.FOLLOWING: {
        ...
        this.activeTab = ManageLoggedInUsersFollowsPageComponent.FOLLOWING;
        this.lastPage = null;
      ...
      case this.appData.RouteNames.FOLLOWERS: {
        ...
        this.activeTab = ManageLoggedInUsersFollowsPageComponent.FOLLOWERS;
        this.lastPage = null;
        this.infiniteScroller.reset();
        this.datasource.adapter.reset();
        break;
  ...
  constructor(
    ...
    this.loggedInUserSubscription = this.appData.loggedInUserObservable.subscribe((loggedInUserObservableResult) => {
      ...
      if (!loggedInUserObservableResult.isSameUserAsBefore) {
        this.datasource.get(0, ManageLoggedInUsersFollowsPageComponent.PAGE_SIZE, null);
      }
    });
  ...
  infiniteScroller: InfiniteScroller = new InfiniteScroller(
    ...
    ManageLoggedInUsersFollowsPageComponent.WINDOW_VIEWPORT,
    ManageLoggedInUsersFollowsPageComponent.BUFFER_SIZE
  );
