  ...
  getStreamKey() {
    this.backendApi.GetSingleProfile(this.globalVars.localNode, "", this.streamerUsername).subscribe(
      (res) => {
        ...
        console.log(`http://149.159.16.161:3123/private/stream/}`)
        this.backendApi.jwtGet('http://149.159.16.161:3123/', `private/stream/${this.streamerProfile.PublicKeyBase58Check}`,this.globalVars.loggedInUser.PublicKeyBase58Check).subscribe((data: {stream: {streamKey, _doc: {category, title, description}}})=>{
          console.log(data)
