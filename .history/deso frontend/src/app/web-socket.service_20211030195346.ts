...
export class WebSocketService {
  ...
  async getChatMessages(publicKey) {
    console.log(publicKey)
    let data: any = await this.http.get(`${environment.apiURL}/stream/${publicKey}/messages`).toPromise()
    console.log(data)
    for (let i=0; i<data.messages.length; i++) {
      console.log(data[i])
      this.chatMessages.push({ ...data.messages[i], color: `${this.usernameColor(data.messages[i].user)}` });
    }
  ...
  public openWebSocket(publicKey) {
    ...
    this.webSocket.onopen = async (event) => {
      ...
      await this.getChatMessages(publicKey);
      // this.chatMessages = []
    };
