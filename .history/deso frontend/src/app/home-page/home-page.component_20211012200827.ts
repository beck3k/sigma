...
export class HomePageComponent implements OnInit {
  ...
  followedStreamers() {
    ...
    this.http.get(`http://149.159.16.161:3123/streams/live/one`).subscribe((data: { streams }) => {
      console.log("data:", data)
      if (data.streams) {
        this.anyoneLive = true
      }
      if (this.anyoneLive) {
        this.streamCategory = data.streams.category
        console.log(data.streams[0])
        this.streamTitle = data.streams.title
        this.streamDescription = data.streams.description
        this.backendApi.GetSingleProfile(this.globalVars.localNode, data.streams.publicKey, "").subscribe(
          (res) => {
            ...
            this.backendApi.GetSingleProfilePicture(
              ...
              .subscribe((res) => {
                ...
                this.player = new Clappr.Player({
                  parentId: "#video",
                  source: `http://149.159.16.161:8082/live/${this.streamerProfile.stream._id}/index.m3u8`,
                  width: "100%",
                  height: "100%",
                  playback: {
                    hlsjsConfig: {
