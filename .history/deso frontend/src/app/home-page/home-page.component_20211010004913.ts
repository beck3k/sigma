...
export class HomePageComponent implements OnInit {
  ...
  followedStreamers() {
    this.http.get(`http://149.159.16.161:3123/streams/live/one`).subscribe((data: { stream }) => {
      ...
      if (this.anyoneLive) {
        this.streamCategory = data.stream[this.streamNumber].category
        this.streamTitle = data.stream[this.streamNumber].title
        this.streamDescription = data.stream[this.streamNumber].description
        this.backendApi.GetSingleProfile(this.globalVars.localNode, data.stream[this.streamNumber].publicKey, "").subscribe(
          (res) => {
